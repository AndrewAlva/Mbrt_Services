	...
	// Section ID = 14
		...
		$('#camilaWebCarousel').on('click', '.rightArrow a', function(event) {
			event.preventDefault();
			rightMovementNavigation(currentDisplayCamilaScreen, totalCamilaWebScreens, $('#camilaWebCarousel'), currentSectionId + 1, 'contact');
		...
		$('#camilaWebCarousel').on('click', '.leftArrow a', function(event) {
			event.preventDefault();
			leftMovementNavigation(currentDisplayCamilaScreen, $('#camilaWebCarousel'));
		...
		$('#camilaWebCarousel').on('click', '.singleBar', function(event) {
			event.preventDefault();
			projectToLoad = $(this).data("loadprojectid");
			projectsToSkip = Math.abs(projectToLoad - currentDisplayCamilaScreen);
			
			bottomNavBarClick(projectToLoad, projectsToSkip, currentDisplayCamilaScreen, $('#camilaWebCarousel'), totalCamilaWebScreens);
			...
			if (projectToLoad > currentDisplayCamilaScreen) {
				...
				for (var i = 0; i < projectsToSkip; i++) {
					...
					if (currentDisplayCamilaScreen == 1) {
						$('#camilaWebCarousel').find('.leftArrow').addClass('firstArrow');
					};
				};
				console.log('New currentDisplayCamilaScreen value: '+ currentDisplayCamilaScreen);
			};
		...
		$('#mbrtWrapper').on('mousewheel', function(event) {
			...
			if(canScroll && event.deltaX != 0){
				...
				if (currentSectionId == 14) {
					scrollHorizontalCarousel(event.deltaX, event.deltaY, event.deltaFactor, currentDisplayCamilaScreen, totalCamilaWebScreens, $('#camilaWebCarousel'), currentSectionId + 1, 'contact');
